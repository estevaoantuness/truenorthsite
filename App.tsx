...
// TODO: Ajustar cores da paleta principal aqui se necessário (reflete configuração do index.html)
const COLORS = {
  bg: 'slate-950', // #020817
 ...
 * Animação vetorial do navio entrando no porto via Scroll.
 * Configurada para iniciar quando ~40% da seção estiver visível.
 */
const ShipAnimationComponent = () => {
  const ref = useRef(null);
  // TODO: Ajustar duração e easing da animação do navio aqui
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-6">
              ...
              <button onClick={() => scrollToSection('simulacao-plataforma')} className="text-accent-400 hover:text-accent-300 px-3 py-2 rounded-md text-sm font-medium transition-colors">Simulação</button>
        ...
        {isOpen && (
          ...
          >
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              ...
              <button onClick={() => scrollToSection('simulacao-plataforma')} className="text-accent-400 block w-full text-left px-3 py-2 rounded-md text-base font-medium">Simulação</button>
              ...
              <div key={i} className="flex items-start justify-between p-2.5 bg-slate-950/30 border border-slate-800 rounded hover:bg-slate-800/50 transition-colors cursor-pointer group/item">
                <div className="flex items-start gap-2.5">
                   <div className={`mt-1 w-2 h-2 rounded-full shrink-0 ${
                     alert.type === 'red' ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]' : 
                     alert.type === 'yellow' ? 'bg-yellow-500' : 'bg-green-500'
                   }`}></div>
                   <div>
...
const Hero = () => {
  const scrollToSimulation = (focusArea: 'form' | 'results') => {
    const el = document.getElementById('simulacao-plataforma');
    if (el) {
      el.scrollIntoView({ behavior: 'smooth' });
      // Evento customizado poderia ser disparado aqui para focar o input ou resultado
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-8 animate-fade-in-up">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button 
                onClick={() => scrollToSimulation('form')}
              ...
              <button 
                onClick={() => scrollToSimulation('results')}
...
const HowItWorks = () => {
  // TODO: Ajustar textos principais desta seção
  const steps = [
    { 
      id: 1, 
      title: "Importação dos dados", 
      desc: "Ingestão automática de BL, Invoices, Packing List, NCM e catálogo de produtos.",
      icon: <Globe2 className="w-5 h-5 text-white" />
    },
    { 
      id: 2, 
      title: "Validação Inteligente", 
      desc: "Checagem cruzada de NCM, necessidade de LPCO e consistência de dados tributários.",
      icon: <FileSearch className="w-5 h-5 text-white" />
    },
    { 
      id: 3, 
      title: "Integração Portal Único", 
      desc: "Pré-validação dos dados simulando as regras oficiais da Receita Federal.",
      icon: <ShieldCheck className="w-5 h-5 text-white" />
    },
    { 
      id: 4, 
      title: "Alertas e Risco", 
      desc: "Sugestão de correção e estimativa de economia antes do registro oficial.",
...
const BenefitsSection = () => {
  const benefits = [
    { 
      title: "Redução de Demurrage", 
      value: "Até 40%", 
      sub: "menos incidentes",
      desc: "Menos tempo de conferência significa liberação mais rápida e menos custos de armazenagem.", 
      icon: <Clock /> 
    },
    { 
      title: "Menos multas NCM", 
      value: "Zero", 
      sub: "erros de classificação",
      desc: "Validação preditiva elimina multas por classificação incorreta antes do registro.", 
      icon: <ShieldCheck /> 
    },
    { 
      ...
      value: "-80h", 
      sub: "por mês",
      desc: "Elimine o retrabalho manual e a conferência de planilhas infinitas.", 
      icon: <BarChart3 /> 
    },
    { 
      title: "Previsibilidade", 
      value: "100%", 
      sub: "controle de lead time",
      desc: "Controle centralizado de todos os processos, licenças e custos estimados.", 
      icon: <Container /> 
    },
  ];
                   ...
                   <div className="w-48 h-48 rounded-full border border-slate-800 bg-slate-900/50 flex items-center justify-center relative shadow-[0_0_40px_rgba(2,6,23,0.8)_inset]">
                      ...
                      <svg className="absolute w-full h-full -rotate-90">
                         ...
                         <circle
                           ...
                           strokeDashoffset={2 * Math.PI * 80 * (1 - 0.98)}
                           strokeLinecap="round"
                           className="text-primary-500 drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]"
                         />
                      </svg>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          ...
          <div className="order-2 md:order-1">
            ...
            <div className="space-y-6 text-slate-400 text-lg leading-relaxed">
              <p>
                Somos um time focado em reduzir erro e ociosidade nas importações brasileiras.
                A complexidade da DUIMP não precisa ser um risco para o seu negócio.
              </p>
              <p>
                Combinamos conhecimento aduaneiro profundo, tecnologia de ponta e IA para tornar o processo de importação mais inteligente, previsível e ágil.
              </p>
              <div className="bg-slate-950/50 p-6 border-l-4 border-primary-600 rounded-r-lg">
                <p className="text-white font-medium italic">
                  "Nosso objetivo é dar previsibilidade de custo e tempo para operações que não podem se dar ao luxo de ficar com contêiner parado no porto."
                </p>
              </div>
            </div>
          </div>
  ...
  const personas = [
    {
      ...
      items: ["Visão consolidada de todos os processos", "Alertas de custo em tempo real", "Compliance fiscal automatizado"]
    },
    {
      title: "Trading Companies",
      items: ["Gestão de múltiplos clientes e CNPJs", "Histórico centralizado de NCMs", "Redução de lead-time operacional"]
    },
    {
      title: "Operadores Logísticos",
      items: ["Previsibilidade de atracação", "Integração via API com sistemas legados", "Status unificado para o cliente final"]
    },
    {
      title: "Despachantes Aduaneiros",
      items: ["Validação prévia de documentos", "Menos digitação manual", "Foco em inteligência e consultoria"]
    }
  ];
...
const PlatformSimulation = () => {
  ...
  // TODO: Ajustar lógica de validação e cálculo de impacto da simulação aqui
  const runSimulation = () => {
    setCalculating(true);
    setResults(null);
    setShowReport(false);
    ...
    setTimeout(() => {
      ...
      // 2. Validar Subfaturamento (Simulação tosca: valor < peso * 2)
      ...
      // 4. Demurrage Risk se não houver LPCO solicitado
      if (requiresAnuente && !compliance.lpcoRequested) {
        ...
        impactLow += 10000; // Demurrage base
        impactHigh += 40000;
      }
      ...
      // Formatando valores monetários fictícios
      const fmt = (v: number) => v.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
      
      // Calculando breakdown de savings
      ...
      setResults({
        ...
        totalImpact: fmt(impactHigh + (impactHigh * 0.2)), // Adicionando uma margem
        avoided: fmt(totalAvoidedRaw), // Promessa de economia
        details: {
          fines: fmt(finesPart),
  ...
  return (
    <section id="simulacao-plataforma" className="py-24 bg-slate-900 scroll-mt-20">
      <AnimatePresence>
        {showReport && results && (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-12 text-center">
           ...
           <h2 className="text-3xl font-bold text-white mb-4">Simule o impacto na sua operação</h2>
           <p className="text-slate-400 max-w-2xl mx-auto">
             Preencha os dados abaixo para uma pré-validação simplificada. 
             <span className="block mt-1 text-xs opacity-70">*Esta é uma simulação baseada em regras fictícias para demonstração da Plataforma.</span>
           </p>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
            ...
            <div className="bg-slate-950 p-6 rounded-xl border border-slate-800 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div>
                   ...
                   <select 
                     className="w-full bg-slate-900 border border-slate-700 text-slate-200 text-sm rounded-lg p-2.5 focus:ring-primary-500 focus:border-primary-500"
            ...
            <div className="bg-slate-950 p-6 rounded-xl border border-slate-800 shadow-sm">
              ...
              <div className="space-y-4">
                {items.map((item, index) => (
                  <div key={item.id} className="relative bg-slate-900/50 p-3 rounded border border-slate-800">
                    <div className="grid grid-cols-12 gap-3">
                       <div className="col-span-12 sm:col-span-5">
                          <input 
                            placeholder="Descrição do Produto"
                            className="w-full bg-slate-950 border border-slate-700 text-slate-200 text-xs rounded p-2 focus:border-primary-500 outline-none placeholder:text-slate-600"
                            value={item.desc}
                 ...
                 <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4">
                       <button 
                         ...
                         className={`flex-1 py-2 px-3 rounded text-sm font-medium border transition-colors ${compliance.lpcoRequested ? 'bg-primary-600/20 border-primary-600 text-primary-400' : 'bg-slate-900 border-slate-700 text-slate-400 hover:bg-slate-800'}`}
                       >
                         Sim
                       </button>
                       <button 
                         onClick={() => setCompliance({...compliance, lpcoRequested: false})}
                         className={`flex-1 py-2 px-3 rounded text-sm font-medium border transition-colors ${!compliance.lpcoRequested ? 'bg-slate-800 border-slate-600 text-white' : 'bg-slate-900 border-slate-700 text-slate-400 hover:bg-slate-800'}`}
                       >
                         Não
                       </button>
                    </div>
          ...
          <div className="relative">
             {!results && !calculating && (
               <div className="absolute inset-0 flex items-center justify-center bg-slate-950/50 border border-slate-800 border-dashed rounded-xl z-10 backdrop-blur-[2px]">
    ...
    <section id="contato" className="py-24 bg-slate-950 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        <div className="bg-gradient-to-b from-slate-900 to-slate-950 border border-slate-800 p-8 md:p-12 rounded-2xl shadow-2xl relative overflow-hidden">
          ...
          {/* FUTURO: Adicionar formulários de contato integrados com CRM (HubSpot, RD) */}
...
export default function App() {
  ...
  // FUTURO: Implementar tracking de eventos (Google Analytics / Mixpanel)
  
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 font-sans selection:bg-primary-600 selection:text-white overflow-x-hidden">
      <Navbar />
      <main>
        <Hero />
        <HighLevelFlow />
